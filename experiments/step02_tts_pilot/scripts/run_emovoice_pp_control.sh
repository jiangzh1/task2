...
INPUT="${1:-$STEP/artifacts/official_pp_angry_control.ready.jsonl}"
RUN_NAME="${2:-official_pp_angry_control}"
GPU_ID="${3:-0}"
...
"$PYTHON" examples/tts/inference_tts.py \
  ...
  ++model_config.phn_tokenizer="$STEP/assets/EmoVoice/Qwen2.5-0.5B-phn" \
  ...
  ++ckpt_path="$STEP/assets/EmoVoice/EmoVoice-PP.pt" \
