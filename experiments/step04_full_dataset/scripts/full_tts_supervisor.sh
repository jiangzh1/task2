...
ART="$STEP/artifacts/full_tts_pp"
OUT="$STEP/audio/full_tts_pp"
...
flock -n 9 || { echo "已有 full_tts_supervisor 正在运行"; exit 3; }
exec >>"$LOG" 2>&1

echo "[$(date -Is)] supervisor 启动，batch_size=$LIMIT"
...
while true; do
  ...
  set -e
  if [[ "$code" != "0" ]]; then
    echo "[$(date -Is)] 模型进程异常退出 code=$code；保留现场并停止，避免无休止重载模型"
    exit "$code"
  fi
  "$PY" "$STEP/scripts/manage_full_tts.py" reconcile --db "$DB" --raw-dir "$RAW" --segment-dir "$SEGMENTS"
